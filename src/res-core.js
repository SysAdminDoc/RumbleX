...
const RES_CORE = {
    ...
    settingsManager: {
        ...
        async exportAllSettings() {
            ...
            const commentBlockedUsers = await this.getBlockedUsers('comment');
            const liveChatBlockedUsers = await this.getBlockedUsers('livechat');
            const fullConfig = {
                settings,
                commentBlockedUsers,
        ...
        async importAllSettings(jsonString) {
            try {
                const fullConfig = JSON.parse(jsonString);
                if (typeof fullConfig.settings !== 'object' || !Array.isArray(fullConfig.commentBlockedUsers) || !Array.isArray(fullConfig.liveChatBlockedUsers)) {
                    throw new Error("Invalid config file structure.");
                }
                await this.save(fullConfig.settings);
                await this.saveBlockedUsers(fullConfig.commentBlockedUsers, 'comment');
                await this.saveBlockedUsers(fullConfig.liveChatBlockedUsers, 'livechat');
                return true;
            } catch (error) {
                console.error("[RumbleX] Error importing settings:", error);
                return false;
            }
        }
    },
    ...
    async populateBlockedUsersList(type) {
        ...
        if (users.length === 0) {
            ...
            users.sort((a,b) => a.toLowerCase().localeCompare(b.toLowerCase())).forEach(user => {
                $list.append(`<div class="res-blocked-user-item" data-username="${user.toLowerCase()}"><span>${user}</span><button class="res-button res-unblock-btn" data-username="${user}" title="Unblock ${user}">Unblock</button></div>`);
            });
            $unblockAllBtn.show();
        }
    },
