...
- The test suite now watches for work that piles up as you browse. It runs twenty in-app navigations with the shipped defaults and fails if any observer, timer, listener or page rescan survives a navigation or grows from one lap of pages to the next, and it checks that the same navigations cost nothing at all with every module switched off. Teardown is now checked for all 139 modules rather than the first 75.
