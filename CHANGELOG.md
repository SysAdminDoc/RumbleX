...
- The test suite now runs from a clean checkout. Five specs mounted the extension against a 600 KB capture of a real signed-in session that was never committed, and because they read it at load time they failed outright rather than skipping, so `npm run verify` could not pass for anyone but the maintainer. A reduced version of that capture ships with the repository. It keeps the page structure and replaces every channel name, viewer name, message id, video id, asset URL and CDN path segment with a synthetic value, and drops the capturing browser's own extension artifacts. The secret-policy suite asserts that on the committed bytes, and a build check fails if the file drifts from the script that generates it.
