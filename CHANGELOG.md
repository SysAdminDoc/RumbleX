...
- **A Creator Program panel on channel pages.** Rumble's program counts shorts against a monthly quota, and nothing tells you where you stand. This does, from the listing already embedded in the page you are on, so it makes no request of any kind. Shorts this month are shown against the 20 the program asks for, with other videos and streams alongside. Raids are not counted and the panel says so: they exist only as a chat notice while one is happening, and a counter built on a guessed selector would sit at zero forever. `creatorMode` had been marked "not implemented yet" since it shipped; the list is down to 27.
