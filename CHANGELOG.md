...
- Backup imports and encrypted Gist pulls preserve every local credential that was omitted from the incoming file, including the Live Stream API URL. Preservation now happens inside the serialized commit, so a token changed during a file read or network pull cannot be replaced by an older snapshot. Partial activity restores stay staged and report the failure instead of claiming full success.
...
- The test suite now measures what RumbleX costs every rumble.com page before any feature starts. A trace of the page's own frame times the evaluation of all seven content scripts at document_start on home, watch, search and channel pages. Measured on 2026-09-23 on the maintainer's desktop: 8.4 to 11.1 ms of evaluation per page, about 7 ms of it `content.js`, inside a main-thread task of 11.7 to 15 ms. The test fails above 45 ms of evaluation or a 60 ms task, roughly four times those figures, which catches a regression that multiplies the cost without failing on a busy machine.
- Creators can bring Rumble's own Live Stream API into the Creator Program panel. Paste the API URL from rumble.com/account/livestream-api into Options and turn on Live Stream API Metrics, and the panel shows followers, subscribers and who is watching, plus what this browser has seen since it started: unique chatters, new followers and subscribers, gifted subs, rants and the peak audience. Rumble only ever returns the latest 50 of each list, so repeated polls are merged rather than added up, and the panel says the numbers are locally observed. The URL carries your key, so only rumble.com addresses are accepted, the request is made by the extension in the background with no cookies, the key never reaches the page, and it is left out of ordinary exports. The stream key Rumble includes in the response is dropped before anything else sees it. Polling only happens while the panel is on screen and the tab is visible, at most every 30 seconds. Gifted subs are filed into the rant archive of the video they were bought on, as their own kind with their own count and a kind column in the CSV. Raids are not in the API, and the panel says so.
- Search and channel pages get a filter for the results already loaded. Type part of a title or a channel name, or sort by date, length or views, and the list rearranges without another request to Rumble. The bar says plainly that it only covers what is loaded, results that arrive as you scroll join the current filter and sort straight away, and an empty list tells you whether Rumble returned nothing or your filter hid everything. Sorts the cards cannot answer are greyed out rather than guessed, and turning the feature off puts every result back in Rumble's order.
- Theater Split remembers its layout separately for live streams and recorded videos. Each keeps its own divider position and whether you closed Theater, so widening the chat for a live stream no longer squeezes the next recording. Closing Theater is remembered for that kind of video, and Rumble's own theater button brings it back, with no extra button added to the player. Your existing Split Ratio becomes the starting point for both, and clearing Theater Layout Memory in Options returns to it. An optional Per-Channel Theater Layout setting lets a channel keep a layout of its own that wins over both. Dragging the divider is now saved once when you let go, instead of on every pixel of the drag.
- A failed download now tells you what happened and what to do about it. The panel names the stage that failed, says what kind of failure it was (Rumble refusing, an expired link, sign-in required, the browser blocking the read, a playlist RumbleX could not parse, a conversion the browser could not do, running out of room, or a dropped connection), and offers the next step most likely to work: reload the quality list for fresh links, or save as TS when MP4 conversion failed. A playlist link that Rumble has retired is replaced with a fresh one from the embed payload once, on its own, before anything is reported. None of it retries in a loop, reaches beyond Rumble's own hosts, or asks you to export cookies.
- A frame budget report for tracking down a slow page. Turn on Frame Budget Report under Privacy & Data and RumbleX keeps a local list of its own page scans that ran longer than one frame (16 ms), with the module that ran each one and the kind of page it was on. Export it from the same section. It keeps the last 100 and never leaves the browser unless you export it.
- The test suite now watches for work that piles up as you browse. It runs twenty in-app navigations with the shipped defaults and fails if any observer, timer, listener or page rescan survives a navigation or grows from one lap of pages to the next, and it checks that the same navigations cost nothing at all with every module switched off. Teardown is now checked for every module rather than the first 75.
...
- The test suite now runs from a clean checkout. Five specs mounted the extension against a 600 KB capture of a real signed-in session that was never committed, and because they read it at load time they failed outright rather than skipping, so `npm run verify` could not pass for anyone but the maintainer. The selector harness also refused to run without a second set of private captures. A reduced version of the page capture ships with the repository, and the selector harness now treats the private set as the optional evidence it was always described as. The committed page keeps its structure and replaces everything that identifies a person or a session: names wherever they appear including visible text, the account's follow list, numeric ids inside embedded JSON, chat usernames, the video's title, description, canonical URL and embed id, server-minted tokens, CDN path segments and every asset URL. The secret-policy suite checks each of those classes on the committed bytes, and the file is pinned by a recorded hash so a hand-edited copy fails the build even where the original capture is absent.
...
- First-run extras are explicit opt-ins now. The welcome page no longer preselects layout changes, live buffering, downloads, or full-volume playback, and the action stays disabled until the user chooses something. Live DVR is also described accurately as saving a recent clip rather than rewinding the player.
- Live visual checks now follow Rumble's current watch links, recognize access and Cloudflare verification pages without trying to bypass them, retry transient screenshot failures, and verify the visible chat and related-video surfaces. Frame-preview failures are also recorded in the local diagnostic ring instead of disappearing.
- Store metadata now describes Chromium and Firefox permissions separately, including Firefox MV2's scoped blocking-request APIs and ad hosts. The release gate checks both manifests against that copy.
- Chromium releases now include a signed CRX3 built from the verified Chrome ZIP. The ZIP remains the primary install path because standard Chromium policy can reject direct CRX installation.
- The release gate now verifies the CRX3 developer key, embedded ID, and RSA-SHA256 proof over the exact ZIP payload. A changed signature or payload stops packaging.
- Your RumbleX activity no longer lives in rumble.com's own storage, so clearing that site's data, which browsers offer in one click, no longer wipes watch progress, history, bookmarks, searches, per-channel preferences, comment drafts or rant archives. The extension keeps them in its own storage now. The move happens once, on the first Rumble page after updating: everything is copied, each copy is read back and compared, and only then is the old copy removed, with a snapshot of what was there kept first. A tab closed halfway through simply starts the move again next time, and a copy that does not match is undone with the old data left in charge. Reset All Data clears the moved activity even with no Rumble tab open. Userscripts have no extension storage and keep using the page's storage.
...
- Background messages now pass through a declarative sender and payload registry before dispatch. Rumble content scripts cannot call extension-page operations such as encrypted Gist sync. Download URLs must use HTTPS without embedded credentials, inherited or unknown action names receive an explicit rejection, and unsafe fields are stopped before privileged work starts.
- Settings returned to a Rumble content script now pass through the secret-free transport policy. Extension pages can still read stored credentials for their own controls, while page-connected code cannot receive Gist tokens, Gist IDs, or webhook URLs.
...
- The extension popup now fits Chromium's 600 px popup limit. The header and footer remain visible while the feature catalog scrolls between them, so the last setting and the footer actions are reachable instead of landing below the window.
- Disabled Save actions now use a neutral treatment instead of the same green styling as an available action. Popup screenshots also use Chromium's actual 600 px height, so the checked-in visual evidence matches the installed extension.
- Signed-in My Feed cards use Rumble's current `<rum-card-video>` element. The shared card adapter, selector-health report, filters, progress tools, preview tools, and dark theme now recognize those cards instead of treating a populated feed as broken or leaving black titles on a dark background.
- Watch Progress and Quick Save now mount on the poster wrapper used by signed-in cards instead of trying to append controls to an image element. Full Titles, Title Font, Title Normalizer, and Thumbnail Hider now cover those cards and the floating related grid too.
- The signed-in notification drawer now keeps its heading and notification text readable on every dark palette. Rumble's inline black text no longer wins over the themed panel.
- Live chat and the floating related-video panel now use the active site palette. Related filtering, watched progress, and keyword filtering also recognize Rumble's current floating grid instead of only the retired sidebar structure.
- The first-run screen now stays visible and reports an error when extension storage rejects a read or write. Disabled primary actions also use Windows system colors in forced-colors mode.
- Starter choices and the first-run completion marker now commit in one storage operation. A failed read leaves the working welcome screen visible, and a failed write cannot apply only half of the choice.
- Theater Split in a narrow window keeps the size you give it. Any window resize put the side panel back to 60%, and on a phone that happens every time the address bar slides away, so the panel snapped back as soon as you scrolled. The size you set stays for the rest of the visit now. It still isn't saved, so it never changes what a full-width window opens with.
- Direct downloads in a userscript manager without `GM_download` now save under the name RumbleX gives them. The old fallback clicked a link to Rumble's media server, and browsers ignore a download name on another site's link, so an MP4, TAR or `.m4a` row either opened in the tab or saved under the CDN's own name. RumbleX now fetches the file through the manager's `GM_xmlhttpRequest` and saves that copy by name, and the panel says what it's doing while the file arrives. The copy sits in the tab until it's saved, so this route stops at 512 MB, cancels the transfer and says so in plain words. A manager that can't do either gets told that instead. The tab is never pointed at the file.
- Running the test suite no longer opens a browser window on your desktop. One spec launched a visible Chromium at the top-left corner of the screen on every `npm run verify`, in the middle of whatever you were doing. It runs headless like the rest now, and a new guard fails the build if any spec pins a visible browser again. `RUMBLEX_HEADED=1` still gives you a visible run on purpose.
- `npm run verify` passes on a fresh clone again, on any platform. A Windows checkout wrote every text file with CRLF, so the generated userscripts read as stale and the vendored libraries missed the hashes they are pinned to. Separately, the offline fixture's recorded hash had been taken from a copy with mixed line endings that no checkout reproduces, so it failed everywhere but the machine that made it. Checkouts now use LF, the vendored libraries are never converted, both guards compare content rather than line endings, and a new check proves a CRLF copy of an unchanged tree still passes.
- The copy-link buttons in the download panel no longer show a tick when the browser refused the copy. The clipboard write was never waited for, so a copy that failed because the page was not focused, or clipboard access was denied, still reported success.
- The audio download saves real audio now. Many Rumble videos publish a separate audio-only track, and RumbleX used to ignore it and save the smallest video file instead, under an audio label. The control now saves Rumble's own track as `.m4a` with no conversion, and the metadata sidecar records it as audio. On videos that publish no such track it says so and saves nothing. The Audio Extraction setting finally does something too. `off` hides the control, and `external` copies the track's link for a download tool instead of saving it. The audio row in the deep-scan results saves as `.m4a` as well.
- Low-bitrate audio tracks are no longer thrown away as placeholders. The size check that catches fake CDN responses held named audio to a video floor of about 48 kbps, which a speech-quality track sits under at any length.
- Closing the download panel on Firefox 109 to 123 no longer fires a burst of stale cancellations. Each probe that simply succeeded left a listener behind on the scan, up to 250 of them per scan.
- Downloads no longer offer the seekbar preview strip as a video quality, and no longer throw away a video's audio-only track. Rumble names every format in its own embed payload, and RumbleX was discarding those names and sorting the results out by file size afterwards. The preview strip is a real MP4 at a real path, so it had to be fetched before it could be rejected, and an audio-only rendition is small enough next to a 1080p copy that it was rejected as a placeholder. The names are kept now. The preview strip is skipped without a request ever going out, and an audio-only track is offered as "Audio only".
- Exact Counts now shows when a video was actually published. Rumble only ever says "3 days ago", and the real date is already sitting in the page's own structured data. The label becomes a date and time in your locale and timezone; the relative phrase stays as the label's accessible name, so a screen reader still reads it. Videos whose page carries no usable date are left exactly as Rumble wrote them.
- Clip export and the live DVR save now choose a stream rendition by the same rule as every other download path. Three copies of that decision had drifted apart, so the same video could resolve to a different quality depending on which feature asked for it.
