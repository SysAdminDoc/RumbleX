...
This release is a visual rebuild of the Rumble website itself.

- **The five themes now style the whole site as one system.** Header, search, navigation, category tabs, cards, watch-page actions, chat, comments, menus, forms, and the related-video rail share the selected palette and consistent focus states.
- **Theater Split has a real side-panel control.** Open and hide the panel with visible buttons, resize it with mouse or keyboard, and keep the selected theme across every side-panel tab. The compact layout also works in narrow windows.
- **Player extras no longer cover the video.** Screenshot, Stats, Loop, Bookmark, and Share at time live in one keyboard-accessible Tools menu. It repairs itself when Rumble swaps or delays the real player.
- **Floating controls are one quiet rail.** The page shortcuts now share a compact surface instead of sitting in separate circles.
...
- **Speed Control**. Persistent playback speed (0.25x to 3x) with live stream detection
...
### Downloads & Capture
- **Video Download**. Download direct MP4, HLS-to-MP4, or raw HLS TS. On Chromium desktops with the File System Access picker, large HLS media can convert to **MP4 on disk** through Mediabunny or save as **TS on disk** without retaining the complete stream in memory. Streams whose published size is missing take the disk path too. While Cancel is shown, cancelling stops the conversion and discards partial file changes. The control disappears before the browser starts its final file commit, which cannot be rolled back reliably. Firefox and unsupported userscript environments keep the 512 MiB in-tab ceiling plus direct and TAR fallbacks. Mediabunny 1.55.7 is the default MP4 engine, with mux.js kept as the bounded fallback when WebCodecs, module startup, or conversion is unavailable. RumbleX-owned browser transfers pause on offline events and queue safe ID-only resumes for the next online event without persisting signed media URLs. The automatic **Deep Scan (RUD)** probes `hugh.cdn.rumble.cloud` for quality variants the embed API did not surface, with live progress, per-row copy-link buttons, and TAR live-replay support.
- **Failure guide**. A failed download names the stage that failed (embed metadata, HLS master playlist, the rendition stream, MP4 conversion, writing the file, or the direct MP4 download), says what kind of failure it was (a refusal, an expired link, sign-in required, a browser block on reading the media server, a playlist RumbleX could not read, a conversion the browser could not do, running out of room, or a dropped connection) and offers the one next step most likely to work, with a button where there is one. A stream link that has expired is swapped for a fresh one from the embed payload once, automatically. Nothing retries in a loop, nothing leaves Rumble's own hosts, and nothing touches your cookies.
...
- **Audio-Only Download**. Many Rumble videos publish a separate audio-only track, and this saves it as `.m4a` exactly as Rumble serves it, with no conversion. Videos that don't publish one say so rather than handing you a video file. Set **Audio Extraction** to `external` to copy the track's link for a download tool instead, or to `off` to hide the control.
