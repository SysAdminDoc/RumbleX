...
async function testFirefoxBrowserNamespace() {
    ...
    assert.equal(platform.capabilities.requestBlockingMode, 'firefox-webrequest');
    assert.equal(platform.capabilities.requestBlockingRules, 7);
    assert.equal(platform.capabilities.streamingFileSave, false);
}
...
async function testChromiumCallbackNamespace() {
    ...
    const context = baseContext({ chrome }, { showSaveFilePicker() {} });
    vm.runInContext(ADAPTER, context, { filename: 'platform.js' });
    const platform = context.RumbleXPlatform;
