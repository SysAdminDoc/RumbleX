...
(function () {
    ...
    const META = {
        ...
        debugSelectorTelemetry: { group: 'advanced', label: 'Debug Selector Telemetry', desc: 'Log selector fallbacks and timeouts locally (no network).' },
        // Layout & UI cleanup
        ...
        downloadLiveStreams: { group: 'downloads', label: 'Download Live Streams', desc: 'Opt-in: record live HLS to MP4. High risk of CDN failures.' },
        ...
        channelArchiveFilterClips: { group: 'downloads', label: 'Channel Archive — Skip Clips', desc: 'Exclude clip-tagged videos from archive runs.' },
        channelArchiveMaxItems: { group: 'downloads', label: 'Channel Archive Max Items', desc: 'Per-run cap (default: 50).' },
        // Feed, filtering & moderation
        shortsFilterScope: { group: 'feed-controls', label: 'Shorts Filter Scope', desc: 'everywhere | feedOnly | searchOnly | off.' },
        blockedChannelsMeta: { group: 'feed-controls', label: 'Channel Block Metadata', desc: 'Array of { channel, reason, createdAt, expiresAt }.' },
        blockedKeywordsMode: { group: 'feed-controls', label: 'Keyword Filter Mode', desc: 'literal | regex | wildcard.' },
        filterPreviewBadges: { group: 'feed-controls', label: 'Filter Preview Badges', desc: 'Show local match counts in settings/log.' },
        politicsFilterPreset: { group: 'feed-controls', label: 'Politics Filter Preset', desc: 'off | reduce | hide — optional category preset.' },
        remoteCosmeticRules: { group: 'advanced', label: 'Remote Cosmetic Rules', desc: 'Signed data-only DOM/CSS rules, opt-in.' },
        remoteCosmeticRulesChannel: { group: 'advanced', label: 'Cosmetic Rules Channel', desc: 'stable | preview.' },
        // Live chat & rants
        chatMentionHighlight: { group: 'comments-chat', label: 'Chat Mention Highlight', desc: 'Highlight @mentions of you in chat.' },
        chatClickToMention: { group: 'comments-chat', label: 'Click to Mention', desc: 'Click a username to insert @mention in composer.' },
        chatParticipantsList: { group: 'comments-chat', label: 'Chat Participants List', desc: 'Local participant drawer.' },
        ...
        multiStreamViewer: { group: 'comments-chat', label: 'Multi-Stream Viewer', desc: 'Experimental 2-4 stream grid with independent chat panels.' },
        // Comments
        commentThreadView: { group: 'comments-chat', label: 'Comment Thread View', desc: 'Focus a comment + its replies.' },
        commentSearch: { group: 'comments-chat', label: 'Comment Search', desc: 'In-page search across loaded comments.' },
        commentMuteDurations: { group: 'comments-chat', label: 'Comment Mute Durations', desc: 'Minutes — array, e.g. [1440,10080].' },
        commentExport: { group: 'comments-chat', label: 'Comment Export', desc: 'Export loaded comments to JSON/CSV.' },
    ...
    function normaliseImported(raw) {
        return RXSettingsSchema.normalizeStored(raw, DEFAULTS);
    }
